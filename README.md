...
**Experimental, short-context only:** Laya checkpoints have 512/1024-token contexts, additional instruction/option limits, and a 48-token limit per option description. The bridge rejects requests that Laya would truncate. Default router instructions and structured profiles can exceed these limits; this is not a drop-in replacement for Jev on long conversations. Use compact custom thinking descriptions and short tasks, and expect existing fallback/keep-current behavior when a request does not fit. No cloud evaluator is used on failure, but your configured fallback generation model may still be cloud-hosted.
